...
// loop is used when we have between 8 and 63 bytes to compare.
// Invariants:
//  - original slices contained at least 8 bytes (DX >= 8)
//  - we have at most 63 bytes left to compare (R8 < 64)
loop:
	CMPQ	R8, $16
	JB  _0through15
	// X0 = a[:16]
...
small:
	...
	// if uinptr(unsafe.Pointer(&a[0]) > 0xF8 {
	//   goto si_high
	// }
	CMPB    SI, $0xf8
	JA  si_high
...
si_finish:
	...
	// if uinptr(unsafe.Pointer(&b[0]) > 0xF8 {
	//   goto di_high
	// }
	CMPB	DI, $0xf8
	JA	di_high
...
// loop is used when we have at least 64 bytes to compare. It is similar to
