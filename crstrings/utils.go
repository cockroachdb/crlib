...
// PrependIfNotEmpty prepends the given prefix to the string if it is not empty.
//
// Sample usage:
//
...
// AppendIfNotEmpty appends the given suffix to the string if it is not empty.
//
// Sample usage:
//
//	fmt.Sprintf("%s%s", crstrings.AppendIfNotEmpty(a, ", "), b)
func AppendIfNotEmpty(s, suffix string) string {
	if s == "" {
		return ""
	}
	return s + suffix
